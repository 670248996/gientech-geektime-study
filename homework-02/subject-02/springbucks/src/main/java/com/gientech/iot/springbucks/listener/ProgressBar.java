package com.gientech.iot.springbucks.listener;
...
public class ProgressBar {
     ...
     */
    public void printUploadPercentProcess(long bytesRead, long contentLength) {
        ...
        String content = ColorEnum.YELLOW.getValue() + bar + percentRadio;
     ...
     * @param contentLength 内容长度
     */
    public void printUploadByteProcess(long bytesRead, long contentLength, UnitEnum unitEnum) {
        int percent = (int) (bytesRead * 100.0 / contentLength);
        if (startPercent < percent) {
            for (int i = startPercent; i < percent; i++) {
                stringBuilder.replace(i, Math.min(TOTAL, i + 1), String.valueOf(FRONT_GROUND));
     ...
     * @param contentLength 内容长度
     */
    public void printResolvePercentProcess(long bytesRead, long contentLength) {
        int percent = (int) (bytesRead * 100.0 / contentLength);
        if (startPercent < percent) {
            for (int i = startPercent; i < percent; i++) {
                stringBuilder.replace(i, Math.min(TOTAL, i + 1), String.valueOf(FRONT_GROUND));
        ...
        content += bytesRead == contentLength ? "(校验成功)" : "(校验中)";
        // 输出绿色进度条
        System.out.print(content);
    }
